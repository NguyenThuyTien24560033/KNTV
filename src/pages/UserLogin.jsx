...
const UserLogin = ({ onBack }) => {
  ...
  const registerUser = async () => {
    try {
      ...
      const check = await fetch(`${API}?email=${email}`);
      ...
      // Đăng ký xong chuyển sang form Đăng nhập
      setIsLogin(true);
      return data;
    } catch (err) {
  ...
  const loginUser = async () => {
    try {
      ...
      return data[0]; // lấy user đầu tiên
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isLogin) {
        ...
        //nếu đã có user
        if (user) {
          // Lưu thông tin vào trình duyệt để dùng cho các trang khác
          //fake token
          ...
          // Phát sự kiện để Header nhận biết user đã đăng nhập
          window.dispatchEvent(new Event("userChanged"));
          // Chuyển về trang chủ
          toast.success("Login success!");
          navigate("/");
          // Đóng modal nếu có modal là cái gì
          if (onBack) onBack();
        }
        //nếu chưa có user
      } else {
